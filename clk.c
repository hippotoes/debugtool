...
int fclk_test(int fclk)
{
    ...
    if (OD > 3)
        OD = 3;
    M = ((fclk << OD) / reference_freq) - 2;
    // printf("fclk value= %d  ;  OD value= %d  ;  M value= %d \n",fclk,OD,M);
    fclk_cal_test = (27 * (M + 2)) >> OD;
    // printf("fclk_cal_test value= %d \n",fclk_cal_test);
    return fclk_cal_test;
}

void set_evclk(unsigned int frequence1, unsigned int frequence2)
{
    unsigned int j;
    unsigned char pBuffer[FP_SECTOR_SIZE];
    ...
    sf_sector_erase(0x10000);
    pBuffer[96] = frequence1 % 256;
    ;
    pBuffer[97] = (frequence1 >> 8) % 256;
    pBuffer[98] = (frequence1 >> 16) % 256;
    pBuffer[99] = (frequence1 >> 24) % 256; // 0x10060

    pBuffer[112] = frequence2 % 256;
    ;
    pBuffer[113] = (frequence2 >> 8) % 256;
    pBuffer[114] = (frequence2 >> 16) % 256;
    pBuffer[115] = (frequence2 >> 24) % 256; // 0x10070
    for (j = 0; j < FP_SECTOR_SIZE; j += 4)
    {
        sf_write_data(0x10000 + j, *(unsigned int *)(pBuffer + j), 4);
    }
...
{
    printf("  clk d/e/v/m: clock d/e/v/m read.\n");
